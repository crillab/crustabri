...
pub struct FlatABACompleteConstraintsSolver<T>
where
    T: LabelType,
{
...
impl<T> FlatABAConstraintsEncoder<T> for FlatABACompleteConstraintsSolver<T>
where
    T: LabelType,
{
    fn new(
        ...
        let mut af = cycle_breaker.remove_cycles(init_af);
        ...
        Self { af, solver, varmap }
    }

    fn assignment_to_extension(&self, assignment: &Assignment) -> Vec<&Argument<T>> {
        let mut extension = Vec::new();
        for assumption in self.af.assumption_ids() {
            if assignment.value_of(self.varmap.assumption_var(*assumption).unwrap().var())
                == Some(true)
            {
                extension.push(self.af.argument_set().get_argument_by_id(*assumption));
            }
        }
        extension
    }
...
impl<T> CredulousAcceptanceComputer<T> for FlatABACompleteConstraintsSolver<T>
where
    T: LabelType,
{
    fn are_credulously_accepted(&mut self, args: &[&T]) -> bool {
        ...
        let lits = args
            ...
            .map(|arg| {
                self.varmap
                    .atom_is_applied_var(arg.id(), self.solver.as_mut())
            })
            .collect::<Vec<_>>();
        self.solver
...
mod tests {
    ...
    fn assert_extensions(str_af: &str, expected: Vec<Vec<usize>>) {
        ...
        let mut encoder = FlatABACompleteConstraintsSolver::new(
            &af,
            sat::default_solver(),
        ...
        loop {
            match encoder.solver.solve() {
                SolvingResult::Satisfiable(assignment) => {
                    models.push(
                        encoder
                            .assignment_to_extension(&assignment)
                            .iter()
                            .map(|l| *l.label())
                            .collect::<Vec<_>>(),
                    );
                    ...
                    for assumption in af.assumption_ids() {
                        let solver_var = encoder.varmap.assumption_var(*assumption).unwrap();
                    ...
                    encoder.solver.add_clause(blocking_cl);
                }
